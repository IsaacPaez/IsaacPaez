...
router.post("/start-whatsapp", async (req, res) => {
    ...
    try {
        ...
        const numberData = user.whatsappNumbers.find(n => n._id.toString() === numberId);
        if (!numberData) {
            console.error(`❌ Número no encontrado en la base de datos para numberId: ${numberId}`);
        ...
        client.on("disconnected", async (reason) => {
            console.log(`⚠️ WhatsApp desconectado para ${numberData.number}. Motivo: ${reason}`);
            ...
            try {
                ...
                console.error("❌ Error actualizando el estado en la base de datos:", error);
            }
        
            // 🔄 **Cerrar Puppeteer de forma segura antes de eliminar la sesión**
            try {
                if (clients[numberId] && clients[numberId].puppeteer) {
            ...
            // **Eliminar la sesión en memoria, pero NO borrar la carpeta de autenticación**
            delete clients[numberId];
            console.log(`🚪 Sesión de WhatsApp cerrada para ${numberData.number}, pero el backend sigue funcionando.`);
        });
        ...
        client.on("message", async (msg) => {
            ...
            try {
                ...
                console.log(`🔍 Buscando configuración para el número de WhatsApp: ${clientNumber}`);
        
                // 📌 Buscamos en la base de datos al usuario que tenga este número registrado
                const user = await User.findOne({ "whatsappNumbers.number": clientNumber });
        
                if (!user) {
                    console.log("❌ Este número de WhatsApp no está registrado en la base de datos.");
                    return;
                }
        
                const number = user.whatsappNumbers.find(n => n.number === clientNumber);
                ...
                const aiResponse = await getAIResponse(number.aiPrompt, msg.body, user.username);
...
router.post("/stop-whatsapp", async (req, res) => {
    ...
    if (!clients[number]) return res.status(404).json({ message: "No hay sesión activa" });
