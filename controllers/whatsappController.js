...
exports.startWhatsAppSession = async (req, res) => {
    ...
    client.on("message", async (message) => {
        ...
        try {
            ...
            console.log(`🔍 Buscando configuración para el número receptor: ${botNumber}`);
    
            // 📌 Buscar el usuario dueño del número receptor (bot)
            let user = await User.findOne({ "whatsappNumbers.number": botNumber });
    
            if (!user) {
                console.log(`❌ El número ${botNumber} no está registrado en la base de datos.`);
                return;
            }
    
            // 📌 Buscar el número en la lista de WhatsAppNumbers
            let numberEntry = user.whatsappNumbers.find(n => n.number.replace(/\D/g, "") === botNumber);
            ...
            console.log(`✅ Mensaje guardado en la base de datos para el número: ${botNumber}`);
